# -*- coding: utf-8 -*-
"""
Created on Wed Mar 27 11:10:37 2019
...
class MatrixReader(multiprocessing.Process):
    ...
    def run(self):
        ...
        Results = '/'.join(ndumpFolder.split('/')[:-3]) + '/Results/Yaveroglu/'
        print Results
        
        with open(Results+'correlMat_%s.csv' % dist_type[testMode], 'w') as to_write:            
        ...
        while not self.kill_received:
            ...
            try:
                ...
                with open(Results +'correlMat_%s.csv' % dist_type[testMode], 'a') as to_write:            
                    csvw = csv.writer(to_write, delimiter = ';')         
                    temp = []
                    for line in correlMat:
                        for elem in line:
                            temp.append(elem)
                    csvw.writerow([ndumpName.split('/')[-1]] + temp)
...
def saveDistanceMatrix(matrix, networkNames, outputFile):
    ...
    networkNames = [name.split('/')[-1] for name in networkNames]
...
if __name__ == "__main__":
    ...
    if testMode in [7, 10, 14, 16]:
        ...
        computeCorrelDist(corrMats, os.path.join(ndumpFolder+'../Results_yaveroglu/', '{0}.txt'.format(distType)))
